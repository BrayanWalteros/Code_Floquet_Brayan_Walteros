...
def imagen(Delta,varphi,config):
    ...
    ax.set_title(f"Espectro a Δ = {Delta*1000:.3f} meV")
    ax.set_xlabel('$aq_x$')
    ...
    plt.tight_layout()
    plt.savefig(f"band_structure_Delta_{Delta*1000:.3f}_apilamiento_{apilamiento}.png")
    plt.close()
    ...
    fig.suptitle(f'Curvaturas de Berry a Δ = {Delta*1000:.3f} meV\nChern: {chern_str2}', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(f"berry_curvature_Delta_{Delta*1000:.3f}_apilamiento_{apilamiento}.png")
    plt.close()
